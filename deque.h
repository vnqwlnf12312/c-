...
template < typename T >
  template < bool has_default_const >
  void Deque < T > ::construct(bool from_int,
    const Deque < T > * other,
      const T *
        elem, size_t size) {
    if (size == 0) {
    ...
    try {
      for (; i < size; ++i) {
        size_t row_index_ = from_int ? i / row_size_ : other -> begin_.row_ -
          other -> array_ + (i + other -> begin_.index_) / row_size_;
        size_t elem_index_ = (i + (from_int ? 0 : other -> begin_.index_)) % row_size_;
        if (i == 0 || elem_index_ == 0) {
          *(array_ + row_index_) =
          reinterpret_cast < T * > (new char[row_size_ *
            sizeof(T)]);
        }
        if (from_int) {
      ...
      begin_ = from_int ? iterator(array_, 0) : iterator(array_ + (other -> begin_.row_ -
        other -> array_), other -> begin().index_);
      end_ = from_int ? iterator(array_ + size / row_size_, size % row_size_) : iterator(
        array_ +
        (other -> end_.row_ -
          other -> array_),
        other -> end().index_);
      ...
      for (size_t j = 0; j < i; ++j) {
        size_t row_index_ = from_int ? j / row_size_ : other -> begin_.row_ -
          other -> array_ + (j + other -> begin_.index_) / row_size_;
        size_t elem_index_ = (j + (from_int ? 0 : other -> begin_.index_)) % row_size_;
...
template < typename T >
  Deque < T > ::Deque(const Deque < T > & other): array_(reinterpret_cast < T ** > (new char[sizeof(T * ) *
    (other.capacity_)])),
...
template < typename T >
  Deque < T > ::Deque(int size,
    const T & element): array_(reinterpret_cast < T ** > (new char[sizeof(T * ) *
    (size + row_size_) /
    row_size_])),
  size_(size), capacity_((size + row_size_) /
    row_size_) {
    construct < false > (true, nullptr, & element, size);
  }

template < typename T >
  Deque < T > ::Deque(int size): array_(reinterpret_cast < T ** > (new char[sizeof(T * ) * (size +
      row_size_) /
    row_size_])),
  size_(size), capacity_((size + row_size_) /
    row_size_) {
  ...
  void Deque < T > ::push_back(const T & elem) {
    ...
    if (end_.row_ == array_ + capacity_ - 1 && end_.index_ == row_size_ - 1) {
      ...
      try {
        ...
        new_arr[capacity_ + (i.row_ - array_)] = * i.row_;
        *(new_arr + capacity_ + (end_.row_ - array_) + 1) = reinterpret_cast < T * > (new char[row_size_ * sizeof(T)]);
        new( * (new_arr + capacity_ + (end_.row_ - array_)) + row_size_ - 1) T(elem);
        real_last_row = capacity_ + (end_.row_ - array_) + 1;
        real_first_row += capacity_;
        begin_ = iterator(new_arr + capacity_ + (begin_.row_ - array_),
          begin_.index_);
        ...
        for (size_t j = capacity_; j <= capacity_ + (end_.row_ - array_) + 1; ++j) {
      ...
      if (end_.index_ == row_size_ - 1 && end_.row_ - array_ + 1 > real_last_row) {
  ...
  void Deque < T > ::push_front(const T & elem) {
    ...
    if (begin_.row_ == array_ && begin_.index_ == 0) {
      ...
      try {
        ...
        *(new_arr + capacity_ - 1) = reinterpret_cast < T * > (new char[row_size_ * sizeof(T)]);
        ...
        for (size_t j = capacity_ - 1; j <= capacity_ + (end_.row_ - array_); ++j) {
          delete[] reinterpret_cast < char * > (new_arr[j]);
        }
        delete[] reinterpret_cast < char * > (new_arr);
        throw;
      ...
      if (begin_.index_ == 0 && begin_.row_ - array_ - 1 < real_first_row) {
        *(begin_.row_ - 1) = reinterpret_cast < T * > (new char[row_size_ * sizeof(T)]);
...
template < typename T >
  template < typename U >
  typename Deque < T > ::template iterator_ < U > Deque < T > ::iterator_ < U > ::operator++(int) {
...
template < typename T >
  template < typename U >
  typename Deque < T > ::template iterator_ < U > & Deque < T > ::template iterator_ < U > ::operator += (int delta) {
    if (row_ == nullptr) {
      return * this;
    }
    if (delta >= 0) {
      ...
      index_ = index_ >= static_cast < size_t > (delta) ? index_ - delta : (row_size_ +
        (index_ -
          delta) %
        row_size_) % row_size_;
    }
...
template < typename T >
  template < typename U >
  typename Deque < T > ::template iterator_ < U > Deque < T > ::template iterator_ < U > ::operator + (int delta) const {
...
template < typename T >
  ...
  int Deque < T > ::iterator_ < U > ::operator - (const iterator_ < value_type > & other) {
    return (row_ - other.row_) * row_size_ + (index_ - other
      .index_);
  }

template < typename T >
  template < typename U >
  bool Deque < T > ::iterator_ < U > ::operator < (const typename Deque < T > ::iterator_ < U > & other)
const {
...
template < typename T >
  template < typename U >
  Deque < T > ::iterator_ < U > ::iterator_(U ** row, size_t index): row_(row), index_(index),
  element_( * row + index) {}

template < typename T >
  template < typename U >
  Deque < T > ::iterator_ < U > ::iterator_(): row_(nullptr), index_(0),
  element_(nullptr) {}

template < typename T >
  template < typename U >
  Deque < T > ::iterator_ < U > ::iterator_(const iterator_ < U > & other): row_(other.row_), index_(other.index_), element_(other.element_) {}

template < typename T >
  template < typename U >
  typename Deque < T > ::template iterator_ < U > & Deque < T > ::iterator_ < U > ::operator = (const iterator_ < U > & other) {
    row_ = other.row_;
    index_ = other.index_;
...
template < typename T >
  template < typename U >
  Deque < T > ::iterator_ < U > ::operator iterator_ <
  const U > () {
    Deque < T > ::iterator_ <
      const U > to_return;
    to_return.row_ = row_;
...
template < typename T >
  void Deque < T > ::insert(iterator place,
    const T & elem) {
    if (begin_.row_ == nullptr) {
      push_back(elem);
      return;
    ...
    if (end_.index_ == row_size_ - 1) {
      ...
      place = iterator(array_ + remember_row + ((remember_cap == capacity_) ? 0 :
          remember_cap),
        remember_index);
    } else {
      ++end_;
