...
class Deque {
 ...
 private:
  ...
  void construct(bool from_int, const Deque* other, const T* elem, size_t
  size);
...
template<typename T>
template<bool has_default_const>
void Deque<T>::construct(bool from_int, const Deque<T>* other,
                         const T*
                         elem, size_t size) {
  if (size == 0) {
    return;
...
template<typename T>
Deque<T>::Deque(const Deque<T>& other) :  array_(reinterpret_cast<T**>(new char
[sizeof(T*) *
    (other.capacity_)])),
...
template<typename T>
Deque<T>::Deque(int size, const T& element) : array_(reinterpret_cast<T**>(new char
[sizeof(T*)*
    (size + row_size_) /
    row_size_])),
...
void Deque<T>::push_back(const T& elem) {
  ...
  if (end_.row_ == array_ + capacity_ - 1 && end_.index_ == row_size_ - 1) {
    ...
    try {
      ...
      new_arr[capacity_ + (i.row_ - array_)] = *i.row_;
      *(new_arr + capacity_ + (end_.row_ - array_) + 1) = reinterpret_cast<T*>(new
          char[row_size_ * sizeof(T)]);
    ...
    if (end_.index_ == row_size_ - 1 && end_.row_ - array_ + 1 > real_last_row) {
      *(end_.row_ + 1) = reinterpret_cast<T*>(new
          char[row_size_ * sizeof(T)]);
...
template<typename T>
template<typename U>
typename Deque<T>::template iterator_<U>& Deque<T>::template iterator_<U>::operator+=(int
                                                                                      delta) {
  if (row_ == nullptr) {
    return *this;
  }
...
template<typename T>
template<typename U>
Deque<T>::iterator_<U>::iterator_(const iterator_<U>& other) : row_(other.row_), index_
    (other.index_), element_(other.element_){
}
