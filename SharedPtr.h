...
struct ControlBlockRegular: BaseControlBlock {
  ...
  ControlBlockRegular(U* elem, Alloc alloc = std::allocator<U>(), Deleter deleter = std::default_delete<U>());
  void destroyThis() override;
  void useDeleter() override;
};
}
...
template<typename U, typename Alloc>
template<typename... Args>
ControlBlock::ControlBlockAllocateShared<U, Alloc>::ControlBlockAllocateShared(Alloc alloc, Args&&... args) : alloc_(alloc) {
  new (reinterpret_cast<U*>(elem_)) U(std::forward<Args>(args)...);
}

template<typename T, typename Alloc, typename... Args>
SharedPtr<T> allocateShared(Alloc alloc, Args&&... args) {
  using BlockAlloc = typename std::allocator_traits<Alloc>:: template rebind_alloc<typename ControlBlock::template ControlBlockAllocateShared<T, Alloc>>;
  BlockAlloc block_alloc = alloc;
  auto cb = std::allocator_traits<BlockAlloc>::allocate(block_alloc, 1);
  std::allocator_traits<BlockAlloc>::construct(block_alloc, cb, alloc, std::forward<Args>(args)...);
...
template<typename T>
template<typename U, typename Alloc>
SharedPtr<T>::SharedPtr(ControlBlock::ControlBlockAllocateShared<U, Alloc>* block) : block_(block), elem_(reinterpret_cast<T*>(&block->elem_)) {
  if (block_) {
...
template<typename T>
SharedPtr<T>::SharedPtr(ControlBlock::BaseControlBlock* block, T* elem) : block_(block), elem_(elem) {
  if (block_) {
    ++block_->shared_count_;
  }
  if constexpr (std::is_base_of_v<EnableSharedFromThis<T>, T>) {
...
template<typename U, typename Alloc, typename Deleter>
ControlBlock::template ControlBlockRegular<U, Alloc, Deleter>::ControlBlockRegular(U* elem, Alloc alloc, Deleter deleter) : deleter_(deleter), alloc_(alloc), elem_(elem) {
}

template<typename T>
template<typename U, typename enable_if>
SharedPtr<T>::SharedPtr(U* elem) : block_(new ControlBlock::ControlBlockRegular<U, std::allocator<U>, std::default_delete<U>>(elem)), elem_(elem) {
...
void ControlBlock::ControlBlockAllocateShared<U, Alloc>::useDeleter() {
  using UAlloc = typename std::allocator_traits<Alloc>::template rebind_alloc<U>;
...
void ControlBlock::ControlBlockAllocateShared<U, Alloc>::destroyThis() {
  using SharedAlloc = typename std::allocator_traits<Alloc>::template rebind_alloc<ControlBlockAllocateShared<U, Alloc>>;
...
void ControlBlock::ControlBlockRegular<U, Alloc, Deleter>::destroyThis() {
  using RegularAlloc = typename std::allocator_traits<Alloc>::template rebind_alloc<ControlBlockRegular<U, Alloc, Deleter>>;
  RegularAlloc regular_alloc = alloc_;
  std::allocator_traits<RegularAlloc>::deallocate(regular_alloc, this, 1);
}

template<typename T>
SharedPtr<T>::SharedPtr(const SharedPtr& other) : block_(other.block_), elem_(other.elem_) {
  if (block_) {
    ++block_->shared_count_;
  }
}
...
template<typename T>
SharedPtr<T>::SharedPtr(SharedPtr&& other) : block_(other.block_), elem_(other.elem_) {
...
template<typename T>
template<typename U, typename enable_if>
SharedPtr<T>::SharedPtr(const SharedPtr<U>& other) : block_(other.block_), elem_(other.elem_) {
  static_assert(std::is_base_of_v<T, U> || std::is_same_v<T, U>);
  if (block_) {
    ++(block_->shared_count_);
  }
...
template<typename T>
template<typename U, typename enable_if>
SharedPtr<T>::SharedPtr(SharedPtr<U>&& other) : block_(other.block_), elem_(other.elem_) {
  static_assert(std::is_base_of_v<T, U> || std::is_same_v<T, U>);
  other.block_ = nullptr;
  other.elem_ = nullptr;
}
...
SharedPtr<T>::SharedPtr(Y* elem, const Deleter& deleter) {
  auto* block = new ControlBlock::ControlBlockRegular<Y, std::allocator<Y>, Deleter>(elem, std::allocator<Y>(), deleter);
  block_ = block;
  elem_ = block->elem_;
  ++block_->shared_count_;
  if constexpr (std::is_base_of_v<EnableSharedFromThis<T>, T>) {
...
SharedPtr<T>::SharedPtr(Y* elem, const Deleter& deleter, const Alloc& alloc) {
  using BlockAlloc = typename std::allocator_traits<Alloc>:: template rebind_alloc<typename ControlBlock::template ControlBlockRegular<Y, Alloc, Deleter>>;
  BlockAlloc block_alloc = alloc;
  auto* block = std::allocator_traits<BlockAlloc>::allocate(block_alloc, 1);
  new (block) ControlBlock::ControlBlockRegular<Y, Alloc, Deleter>(elem, alloc, deleter);
  block_ = block;
  elem_ = block->elem_;
  ++block_->shared_count_;
  if constexpr (std::is_base_of_v<EnableSharedFromThis<T>, T>) {
...
void SharedPtr<T>::reset(Y* elem) {
  clear();
  auto* block = new ControlBlock::ControlBlockRegular<Y, std::allocator<Y>, std::default_delete<Y>>(elem);
  ++block->shared_count_;
  block_ = block;
  elem_ = block->elem_;
}
...
template<typename T>
WeakPtr<T>::WeakPtr(const WeakPtr& other) : block_(other.block_), elem_(other.elem_) {
  if (block_) {
    ++block_->weak_count_;
  }
}
...
template<typename T>
WeakPtr<T>::WeakPtr(WeakPtr&& other) : block_(other.block_), elem_(other.elem_) {
  other.block_ = nullptr;
  other.elem_ = nullptr;
}
...
template<typename T>
template<typename U, typename enable_if>
WeakPtr<T>::WeakPtr(const WeakPtr<U>& other) : block_(other.block_), elem_(other.elem_) {
  if (block_) {
    ++block_->weak_count_;
  }
}
...
template<typename T>
template<typename U, typename enable_if>
WeakPtr<T>::WeakPtr(WeakPtr<U>&& other) : block_(other.block_), elem_(other.elem_) {
  other.block_ = nullptr;
  other.elem_ = nullptr;
}
...
template<typename T>
template<typename U, typename enable_if>
WeakPtr<T>::WeakPtr(const SharedPtr<U>& sharedptr) : block_(sharedptr.block_), elem_(sharedptr.elem_) {
  if (block_){
    ++block_->weak_count_;
  }
}
...
template<typename T>
void WeakPtr<T>::construct(T* elem, typename ControlBlock::BaseControlBlock* block) {
  if (block_) {
    clear();
  }
  block_ = block;
