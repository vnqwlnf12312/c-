...
bool Line::operator==(const Line& second) const {
  return IsSame(A * second.B, second.A * B)
      && IsSame(B * second.C, C * second.B);
}
...
bool Polygon::isCongruentTo(const Shape& second) const {
  ...
  for (size_t i = 0; i < size2; ++i) {
    ...
    for (size_t j = 0; j < size2; ++j) {
      ...
      vector3 = second_copy->vertices[(i + j) % size2]
          - second_copy->vertices[(i + j + size2 - 1) % size2];
      vector4 = second_copy->vertices[(i + j + 1) % size2]
          - second_copy->vertices[(i + j) % size2];
      vector_prod1 = (vector1).VectorProd(vector2);
      vector_prod2 = (vector3).VectorProd(vector4);
      if (!IsSame(std::abs(vector_prod1 / ((hypot(vector1.x, vector1.y)
          ...
          || !IsSame(hypot(vector2.x, vector2.y),
                     hypot(vector4.x, vector4.y))) {
        is_congruent = false;
        break;
      }
    ...
    for (size_t j = 0; j < size2; ++j) {
      vector1 = vertices[j] - vertices[(j + size2 - 1) % size2];
      vector2 = vertices[(j + 1) % size2] - vertices[j];
      vector3 = second_copy->vertices[(i + size2 - j) % size2]
          - second_copy->vertices[(i + size2 - j + 1) % size2];
      vector4 = second_copy->vertices[(i + size2 - j - 1) % size2]
          - second_copy->vertices[(i + size2 - j) % size2];
      vector_prod1 = (vector1).VectorProd(vector2);
      vector_prod2 = (vector3).VectorProd(vector4);
      if (!IsSame(std::abs(vector_prod1 / ((hypot(vector1.x, vector1.y)
                      * hypot(vector2.x, vector2.y)))),
...
bool Polygon::isSimilarTo(const Shape& second) const {
  ...
  for (size_t i = 0; i < size2; ++i) {
    ...
    for (size_t j = 0; j < size2; ++j) {
      ...
      vector_prod1 = (vector1).VectorProd(vector2);
      vector_prod2 = (vector3).VectorProd(vector4);
      if (!IsSame(vector_prod1 / ((hypot(vector1.x, vector1.y)
                      * hypot(vector2.x, vector2.y))),
...
bool Polygon::containsPoint(const Point& point) const {
  ...
  while (Line(vertices[sides_index], vertices[(sides_index + 1) % size])
      == Line(vertices[sides_index], point)) {
    ++sides_index;
  }
  Point point_on_side =
      (vertices[sides_index] + vertices[(sides_index + 1) % size]) / 2;
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (!side.IsParallel(line_through_point)) {
      intersection = side.Intersection(line_through_point);
      if ((intersection - vertices[i]).ScalarProd(
          intersection - vertices[(i + 1) % size]) < 0
...
bool Ellipse::isCongruentTo(const Shape& second) const {
  ...
  return IsSame(hypot(delta1.x, delta1.y), hypot(delta2.x, delta2.y))
      && IsSame(semi_major_axis, second_copy->semi_major_axis);
}
...
double Rectangle::perimeter() const {
  Point vector1 = vertices[1] - vertices[0],
      vector2 = vertices[2] - vertices[1];
...
Circle Triangle::circumscribedCircle() {
  ...
  Line
      midperpendicular1 = Line(vertices[0], vertices[1]).rotate(mid1, M_PI / 2);
  Line
      midperpendicular2 = Line(vertices[1], vertices[2]).rotate(mid2, M_PI / 2);
...
Circle Triangle::inscribedCircle() {
  Point vector1 = vertices[1] - vertices[0],
      vector2 = vertices[2] - vertices[0], vector3 = vertices[2] - vertices[1];
...
Point Triangle::orthocenter() {
  Line
      line1 = {vertices[0], Line(vertices[1], vertices[2]).Height(vertices[0])};
  Line
      line2 = {vertices[1], Line(vertices[2], vertices[0]).Height(vertices[1])};
...
bool EllipsesEqual(const Ellipse* this_copy, const Shape& second) {
  ...
  return this_copy->focuses().first == second_copy->focuses().first
      ...
      && IsSame(this_copy->SemiMajorAxis(), second_copy->SemiMajorAxis());
}
