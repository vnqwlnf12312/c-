...
template < typename T, size_t N >
  class StackAllocator {
    ...
    ~StackAllocator();
    private: StackStorage < N > * storage_;
  };
...
template < typename T, size_t N >
  template < typename...Args >
  void StackAllocator < T, N > ::construct(T * pointer,
    const Args & ...args) {
    new(pointer) T(args...);
  }
...
template < typename T, class Allocator >
  List < T, Allocator > ::Node::Node(List < T, Allocator > ::BaseNode * prev, List < T, Allocator > ::BaseNode * next,
    const T & value): BaseNode(prev, next), value(value) {}
...
template < typename T, class Allocator >
  ...
  void List < T, Allocator > ::construct(bool from_int, size_t size,
    const T * elem,
      const List * other) {
    if (size == 0) {
      return;
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(const Allocator & alloc): end_(BaseNode {
    & end_, & end_
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(const List < T, Allocator > & other): end_(BaseNode {
    & end_, & end_
  }), size_(0) {
...
template < typename T, class Allocator >
  template < typename U >
  List < T, Allocator > ::iterator_ < U > ::operator iterator_ <
  const U > () const {
