...
void indexes_without_forbidden(int cur_array, int* current_indexes, int* sizes,
                               int& argc, long long& answer, int** arrays) {
  ...
  for (int i = 0; i < size; ++i) {
    if (i_not_in_current(current_indexes, cur_array, i)) {
      ...
      indexes_without_forbidden(cur_array + 1, current_indexes, sizes,
                                argc, answer, arrays);
    }
  }
}
...
int main(int argc, const char* argv[]) {
  ...
  indexes_without_forbidden(0, current_indexes, sizes, argc,
                            sum, arrays);
  std::cout << sum;
  for (int i = 0; i < argc - 1; ++i) {
    delete[] arrays[i];
