...
template < typename T, typename Allocator >
  class List {
    ...
    void construct(bool from_int, size_t size,
      const T * elem,
        const List * other);

    public: template < typename Key,
    typename Value,
    typename Hash,
    typename Equal,
    typename Alloc >
...
template < typename Key, typename Value, typename Hash = std::hash < Key > ,
  ...
  const Key, Value >>>
    class UnorderedMap {
      public: using NodeType = std::pair <
        const Key, Value > ;
      UnorderedMap() =
      default;
      ...
      private: using NodeTypeAlloc =
        typename std::allocator_traits < Alloc > ::template rebind_alloc < NodeType > ;
      public: using iterator = typename List < NodeType,
      Alloc > ::iterator;
      using const_iterator = typename List < NodeType,
      ...
      std::pair < iterator,
      bool > insert(const NodeType & value);
      std::pair < iterator,
      bool > insert(NodeType && value);
      iterator begin();
      const_iterator begin() const;
      const_iterator cbegin() const;
      iterator end();
      ...
      List < NodeType,
      Alloc > list_;
      std::vector < typename List < NodeType,
      Alloc > ::BaseNode * > array_;
      float max_load_factor_ = 0.5;
    };

template < typename T, typename Allocator >
  template < typename U >
  typename List < T, Allocator > ::BaseNode * List < T, Allocator > ::iterator_ < U > ::getNode() {
    return cur_node_;
  }

template < typename T, class Allocator >
  List < T, Allocator > ::BaseNode::BaseNode(List::BaseNode * prev,
    List::BaseNode * next): prev(prev), next(next) {}

template < typename T, class Allocator >
  List < T, Allocator > ::BaseNode::
  BaseNode(List < T, Allocator > ::BaseNode && other): prev(std::move(other.prev)),
...
template < typename T, class Allocator >
  ...
  List < T, Allocator > ::BaseNode::operator = (List < T, Allocator > ::BaseNode && other) {
    if (other.prev == & other) {
      prev = this;
      next = this;
    } else {
...
template < typename T, class Allocator >
  List < T, Allocator > ::Node::Node(List < T, Allocator > ::BaseNode * prev, List < T, Allocator > ::BaseNode * next, int hash): BaseNode(prev, next), value() {
    BaseNode::hash = hash;
  }
...
template < typename T, class Allocator >
  List < T, Allocator > ::Node::Node(
    ...
    const T & value, int hash): BaseNode(prev, next), value(value) {
    BaseNode::hash = hash;
  }

template < typename T, class Allocator >
  List < T, Allocator > ::List(): end_(BaseNode {
    & end_, & end_
  }), size_(0), node_alloc_(Allocator()) {}
...
template < typename T, class Allocator >
  ...
  void List < T, Allocator > ::construct(bool from_int,
    size_t size,
    const T * elem,
      const List * other) {
    if (size == 0) {
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(
    const Allocator & alloc): end_(BaseNode {
    & end_, & end_
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(
    size_t size,
    const Allocator & alloc): List(alloc) {
    construct < true > (true, size, nullptr, nullptr);
  }
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(
    const List < T, Allocator > & other): end_(BaseNode {
    & end_, & end_
  }), size_(0) {
    node_alloc_ =
...
template < typename T, class Allocator >
  List < T, Allocator > ::List(
    List < T, Allocator > && other): end_(std::move(other.end_)),
...
template < typename T, class Allocator >
  template < typename U >
  List < T, Allocator > ::iterator_ <
  U > ::iterator_(const List < T, Allocator > ::iterator_ < U > & other): cur_node_(other.cur_node_) {}
...
template < typename T, class Allocator >
  void List < T, Allocator > ::insertNode(const_iterator it,
    BaseNode * node) {
...
template < typename Key, typename Value, typename Hash, typename Equal,
  typename Alloc >
  UnorderedMap < Key, Value, Hash, Equal, Alloc > ::UnorderedMap(
    const UnorderedMap < Key, Value, Hash, Equal, Alloc > & other): max_load_factor_(other.max_load_factor_) {
...
template < typename Key, typename Value, typename Hash, typename Equal,
  typename Alloc >
  UnorderedMap < Key, Value, Hash, Equal, Alloc > ::UnorderedMap(
    UnorderedMap < Key, Value, Hash, Equal, Alloc > && other): list_(std::move(other.list_)),
...
UnorderedMap < Key, Value, Hash, Equal, Alloc > ::find(const Key & key) {
  ...
  try {
    ...
    for (typename List < NodeType, Alloc > ::iterator it = array_[index]; it != list_.end() && it.getNode() -> hash % array_.size() == index;
      ++it) {
...
UnorderedMap < Key, Value, Hash, Equal, Alloc > ::find(const Key & key) const {
  ...
  try {
    ...
    for (typename List < NodeType, Alloc > ::const_iterator it = array_[index]; it != list_.end() && it.getNode() -> hash % array_.size() == index;
      ++it) {
      if (Equal {}(it -> first, key)) {
        return it;
      }
